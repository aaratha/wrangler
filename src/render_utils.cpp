...
#include <string>
  ...
  rl::Shader SetupDofShader(int screenWidth, int screenHeight) {
    rl::Shader dofShader(0,
                         TextFormat("resources/shaders/dof.fs", GLSL_VERSION));
    if (dofShader.id == 0) {
    ...
    SetShaderValue(dofShader, GetShaderLocation(dofShader, "radius"),
                   &blurRadius, SHADER_UNIFORM_FLOAT);
  ...
  void RenderShadowMap(Shader shadowShader, RenderTexture2D & shadowMap,
    ...
    SetShaderValueMatrix(shadowShader,
                         GetShaderLocation(shadowShader, "lightVP"),
                         lightViewProj);
