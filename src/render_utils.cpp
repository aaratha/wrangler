...
void update_camera(GameState &GameState) {
  float fov_ext = 70.0;
  float fov_rest = 60.0;
  GameState.camera.target.x = lerp_to(GameState.camera.target.x, GameState.player->com.x, 0.2f);
  GameState.camera.target.z = lerp_to(GameState.camera.target.z, GameState.player->com.z, 0.2f);
  ...
  if (GameState.itemActive == 0) {
    if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) && GameState.camera.fovy < fov_ext) {
      GameState.camera.fovy = lerp_to(GameState.camera.fovy, fov_ext, 0.1f);
    } else if (!IsMouseButtonDown(MOUSE_BUTTON_LEFT) && GameState.camera.fovy > fov_rest) {
...
bool is_in_camera_view(const Vector3 &position, float radius, const Camera &camera, int screenWidth,
  ...
  // the object's radius
  bool is_visible = (screenPos.x + radius > 0) && (screenPos.x - radius < screenWidth) &&
...
rl::Shader SetupDofShader(int screenWidth, int screenHeight) {
  ...
  SetShaderValue(dofShader, GetShaderLocation(dofShader, "resolution"), resolution,
                 SHADER_UNIFORM_VEC2);
...
void DrawGUI(GameState &GameState, int &screenWidth, int &screenHeight) {
  ...
  GuiToggleGroup((Rectangle){static_cast<float>(screenWidth - width - margin),
                             static_cast<float>(screenHeight - (4.05 * height) - margin), width,
                             height},
                 "#1#\n#3#\n#8#\n#23#", &GameState.itemActive);
  GuiLabel((Rectangle){static_cast<float>(width / 2 + margin), static_cast<float>(margin),
